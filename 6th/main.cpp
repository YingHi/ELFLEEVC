...
RECT rect_target = { 50L, 50L, 150L, 150L }; // 왼쪽 상단 좌표 (50, 50)에서 오른쪽 하단 좌표 (150, 150)까지의 사각형
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		if (isKeyPressed)
		{
			...
			{
				case 1:
					rect_user.top -= moveDistance;
					rect_user.bottom -= moveDistance;
					if (rect_user.top <= 0) {
						rect_user.top = 0.5;
						rect_user.bottom = 5.5;
					}
					break;
				case 2:
					rect_user.top += moveDistance;
					...
					break;
				case 3:
					rect_user.left -= moveDistance;
					rect_user.right -= moveDistance;
					if (rect_user.left <= 0) {
						rect_user.left = 0.5;
						rect_user.right = 5.5;
					}
			...
			if (rect_user.top >= 50 && rect_user.bottom <= 150 && rect_user.left >= 50 && rect_user.right <= 150) {
				text = L"Fuck You";
			}
			else {
				text = L"Crash!!!";
